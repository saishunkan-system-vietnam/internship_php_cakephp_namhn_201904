...
<!--    <form action="--><?//= URL ?><!--users/add" id="Users" method="post">-->
        <form><table class="table table-hover table-bordered ">
            <tr>
            ...
            <tr>
                <th>Nhóm</th>
                <th>
                    <select name="group" class="selectpicker form-control" multiple title="Chọn một nhóm dưới đây">
                        <?php foreach ($dataGroup as $key => $value) {?>
                        <option style="font-weight: bold;font-size: 17px;" value="<?= $value->id ?>"><?= $key+1?> : <?= $value->name ?></option>
                        <?php }?>
                    </select>
                </th>
            </tr>
